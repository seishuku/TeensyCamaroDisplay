...
__attribute__ ((section (".vectortable"))) const tVectorTable __vect_table=
{
	&__SP_INIT,					//	0x00	ivINT_Initial_Stack_Pointer
	{
		...
		&can_isr,				//	0x2D	ivINT_CAN0_ORed_Message_buffer
...
{
	IRQ-=16;											// Subtract 16 from the IRQ number, IRQs < 16 are special cases, actual IRQs start at 16.
	NVIC_IP(IRQ)=Priority;								// Set IP register with IRQ priority.
	NVIC_ISER(IRQ>>5)|=NVIC_ISER_SETENA(1<<(IRQ&31));	// Select proper ISER register for IRQ number and set that bit to enable it.
}
